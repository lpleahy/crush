...
func DefaultKeyMap(cfg *config.Config) KeyMap {
	...
	// Search / copy-mode (decoupled from the keybindings catalog — direct bindings)
	km.Chat.Search = key.NewBinding(
		key.WithKeys("/"),
		key.WithHelp("/", "search"),
	)
	km.Chat.SearchNext = key.NewBinding(
		key.WithKeys("n"),
		key.WithHelp("n", "next match"),
	)
	km.Chat.SearchPrev = key.NewBinding(
		key.WithKeys("N"),
		key.WithHelp("N", "prev match"),
	)
	km.Chat.CopyMode = key.NewBinding(
		key.WithKeys("v", "V"),
		key.WithHelp("v", "copy mode"),
	)
